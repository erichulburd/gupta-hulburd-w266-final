...
from bert import modeling, optimization, tokenization, run_squad
...
# Required parameters
flags.DEFINE_string(
    "embeddings_dir", "data/uncased_L-12_H-768_A-12",
    "The directory containing the BERT embedding checkpoints, config, and vocab."
...
flags.DEFINE_string("predict_file", "data/squad/dev-v2.0.json",
    ...
    for entry in input_data:
        for paragraph in entry["paragraphs"]:
            ...
            for qa in paragraph["qas"]:
                ...
                example = run_squad.SquadExample(qas_id=qas_id,
...
def convert_examples_to_features(examples, tokenizer, max_seq_length, doc_stride, max_query_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            feature = run_squad.InputFeatures(unique_id=unique_id,
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        # in memory.
        train_writer = run_squad.FeatureWriter(filename=os.path.join(FLAGS.output_dir, "train.tf_record"),
                                     is_training=True)
    ...
    if FLAGS.do_predict:
        ...
        eval_writer = FeatureWriter(filename=os.path.join(FLAGS.output_dir, "eval.tf_record"),
                                    is_training=False)
        eval_features = []
...
if __name__ == "__main__":
    flags.mark_flag_as_required("embeddings_dir")
    # The config json file corresponding to the pre-trained BERT model.
    BERT_CONFIG_FILE = "%s/bert_config.json" % flags.embeddings_dir
    # "The vocabulary file that the BERT model was trained on."
    VOCAB_FILE = "%s/vocab.txt" % flags.embeddings_dir
    # "Initial checkpoint (usually from a pre-trained BERT model)."
    INIT_CHECKPOINT = "%s/bert_model.ckpt" % flags.embeddings_dir
    tf.app.run()
